...
const Shop = () => {
  ...
  const getProductBrands = async (category: string) => {
    try {
      ...
      if (response.status === 200 && response.data.length > 0) {
        setFilterContent([
          ...filterContent,
          { name: "Marca", values: response.data },
        ]);
      } else {
  ...
  const [currentPage, setCurrentPage] = useState<number>(
    parseInt(localStorage.getItem("page") as string) || 0
  );
  ...
  const [filterContent, setFilterContent] = useState<FilterContent[]>(
    category === "cat" || category === "dog"
      ? [{ name: "Edad", values: ["Cachorro", "Adulto", "Adulto senior"] }]
      : []
  );
        ...
        >
          ...
          <Flex flexDir="column">
            ...
            >
              <Flex justifyContent="space-between" gap={10}>
                ...
                >
                  {category
                    ? "Resultados"
                    : cutKeyword(
                        extractCategory(location.pathname).replace(/%/g, " ")
                      )}
                </Heading>
              ...
              <Text fontSize={{ base: "0.8rem", xl: "1.1rem" }} ml={0}>
                ...
                <Paginator
                  ...
                  handleClick={(page: number) =>
                    getProducts({
                      ...
                      keywords: category
                        ? undefined
                        : extractCategory(location.pathname),
                    })
                  }
                />
              </>
